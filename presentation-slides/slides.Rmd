...
[3] Fuchs *et al.* 2018. *PLOS One*. doi:10.1371/journal.pone.0191002
...
* If possible: staff/participants blinded to biomarker result </br>
(unethical if strong prior evidence of biomarker performance!)
...
3. Effect size: $$\operatorname{Pr}[\operatorname{PD} = 1|\operatorname{AB}_{0/1} = 1, \operatorname{Age}] - \operatorname{Pr}[\operatorname{PD} = 1|\operatorname{AB}_{0/1} = 0, \operatorname{Age}]$$

4. $\rho:=\operatorname{cor}[\operatorname{AB}_{0/1},\operatorname{Age}]$ (for variance inflation factor)

5. Conditional error rates $\alpha=0.05$ (two-sided), $\beta=0.1$ </br>
	( $\beta$ flexible, should depend on projected market value $\leadsto$ utility maximization)
...
Prevalence $\operatorname{Pr}[\operatorname{AB}_{0/1}=1]$
...
* Not too big, otherwise age proxy for biomarker
* **Assumption:** $\rho=0.2$

Dropout rate
* late (post 20 weeks) iatrogenic aborts very rare
...
* With dropout: $n = `r ceiling(n / (1 - rho^2) * (1 + dropout))` \approx 3600$
...
* Problem: CCA not valid under $\mathcal{H}_0$ if dropout hazard not constant

* Solution: add more predictors (e.g. "smoking") to regression model </br>
	$\leadsto$ MAR more plausible 
...
interactive version (RStudio): https://bit.ly/3gPXGuz
